...
//utility function for PRIORITY policy 
struct proc* 
findHighestPriority(struct proc *currProc){
  struct proc *highestProc = currProc;
...
int
getPsStates(struct ps_states *ps){
  ps->count = 0;
  struct ps tempPs;
  acquire(&ptable.lock);
  for(struct proc *p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p != 0){
      tempPs.state = p->state;
      tempPs.pid = p->pid;
      tempPs.priority = p->priority;

      ps->systemPs[ps->count] = tempPs;
      ps->count++;
    }
  }
  release(&ptable.lock);
